...
public class PositionNode extends ParseNode {
  ...
  public int callReceived(int token) {
    ...
    while (true) {
      ...
      if (sent == tokens.rawSize()) {
        int downNodeResult = downNode.callReceived(sent);
  ...
  private int forwardToRightOrUp(int idx) {
    if (hasRightNode()) {
      // idx may be "one past last" (rawSize) -> still let rightNode decide
      if (idx == tokens.rawSize()) {
        int rightNodeResult = rightNode.callReceived(idx);
        return rightNodeResult;
      }

      if (tokens.checkIndex(idx)) {
        int rightNodeResult = rightNode.callReceived(idx);
        if (rightNodeResult < 0 || rightNodeResult >= idx) return rightNodeResult;
